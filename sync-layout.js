...
function getActiveNav(fileName) {
  ...
  if (fileName === "index.php") {
    ...
    nav = nav.replace(`href="${fileName}"`, `href="${fileName}" class="nav__link nav__link--active"`);
  }
...
function syncHeader(filePath, fileName) {
  ...
  content = content.substring(0, headerStart) + header + content.substring(headerEnd + 9); // 9 = length of </header>
...
function syncFooter(filePath, fileName) {
  ...
  content = content.substring(0, footerStart) + footer + content.substring(footerEnd + 9); // 9 = length of </footer>
